...
Gemini browser session cookies are account credentials. Release archives contain an empty `cookies.json` placeholder in the project root. A user can paste a browser cookie export into that file; `.gitignore` excludes it from normal version control.
